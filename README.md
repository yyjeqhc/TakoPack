...
Commands that need the openRuyi spec repository or the local Cargo directory
registry read `takopack.toml` from the current directory first, then from the
user config directory.

```toml
[ruyispec]
local_path = "/root/git/ruyia"

[registry]
local_path = "/root/.local/share/takopack/cargo-registry"
```

Command-line paths override the matching `takopack.toml` setting. For example,
`--registry <DIR>` on `resolve-check` overrides `[registry].local_path`.

## Resolve check and BuildRequires

The recommended BuildRequires path for Rust applications is:

```bash
takopack cargo resolve-check Cargo.toml
```

`resolve-check` resolves the project against the local TakoPack Cargo registry,
then prints a short root-level BuildRequires list derived from Cargo.toml. When
`[ruyispec].local_path` is available, TakoPack also validates that those roots
cover the flattened Cargo.lock dependency set through the RPM `Requires:` closure.
If the resolve step cannot select every package, the root BuildRequires generator
still falls back to Cargo.toml version requirements as a best-effort starting
point.

Useful overrides:

```bash
# Use a registry path different from takopack.toml
takopack cargo resolve-check Cargo.toml --registry /path/to/cargo-registry

# Debug the full Cargo.lock-derived baseline
takopack cargo resolve-check Cargo.toml --buildrequires-mode flattened

# Write a validation report
takopack cargo resolve-check Cargo.toml --buildrequires-report buildrequires.txt
```

## Rust crate repo-check workflow

TakoPack now provides native Rust/Cargo repo validation commands:

```bash
takopack cargo repo-index <SPEC_REPO_DIR> --output repo-index.json

takopack cargo repo-check <CARGO_TOML> \
  --index repo-index.json \
  --check-transitive \
  --json

takopack cargo repo-plan <CARGO_TOML> \
  --index repo-index.json \
  --check-transitive \
  --json
```
